...
#            self.update()
    ...
    def __init__(self):      
        ...
        self.PositionListGroupBox = Dock('Position List', size=(100, 1500))
        ...
        self.GraphingWindowGroupBox = Dock('Graphing Window', size=(1500, 1500))
        ...
        StageControl.addWidget(self.fineSteps, 8, 2, 1, 2)
        ...
        StageControl.addWidget(self.sprintCheckbox, 8, 0, 1, 2)
...
#        self.MeasurementLengthLabel.setText("Estimated length: "+str(np.round(L*calibrationFactor,4)))
    ...
    def startPLMapScan(self):
        if True:#self.SpectrometerWidget.SpectrometerWidget != None:        
            if False:# self.SpectrometerWidget.SpectrometerWidget.ContinousMode:
            ...
            H = self.MapHeight.value()
            H_px = self.MapHeightPixel.value()
                
            W = self.MapWidth.value()
            W_px = self.MapWidthPixel.value()
                
            wl = np.linspace(1.2,3.5,250)#self.SpectrometerWidget.SpectrometerWidget.wl_calibration
                
            offset_x = np.linspace(-W/2,W/2,W_px) 
            offset_y = np.linspace(-H/2,H/2,H_px)
            ...
            for current_offset_y in offset_y:
                curr_y = InitialPosition_y + current_offset_y
                # self.Stage.moveYTo(curr_y)
                for current_offset_x in offset_x:
                    
                    if self.stopPLScan.isChecked():
                        break
                
                    curr_x = InitialPosition_x + current_offset_x
                    # self.Stage.moveXTo(curr_x)
                    ...
                    # data[counter,2:] = spec
                    PLMapSubprocessQueue.put(['spec', curr_x, curr_y, counter-2, np.random.randint(1000,size=len(wl))])
