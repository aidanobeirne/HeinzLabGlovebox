...
import threading
from PyQt5.QtWidgets import qApp, QCheckBox, QFileDialog, QSplitter, QSlider, QWidget, QLabel, QListWidget, QListWidgetItem, QGridLayout, QApplication, QGroupBox, QPushButton, QListView, QAbstractItemView, QProgressBar, QDialog, QComboBox, QLineEdit, QDoubleSpinBox, QPlainTextEdit, QInputDialog
...
class GloveBoxSetupWindow(QWidget):
    ...
    def eventLoop(self):
        ...
        # if self.SpectrometerWidget.ContinousMode:
        #     self.SpecPlot.setData(self.SpectrometerWidget.LatestContinousSpectrum, pen=(0,0,255))
...
#            self.update()
    ...
    def __init__(self):      
        ...
        layout.addWidget(self.loadunloadsamplebtn, 6, 2, 1, 2)
        ...
        layout.addWidget(self.fineSteps, 8, 2, 1, 2)
        ...
        layout.addWidget(self.sprintCheckbox, 8, 0, 1, 2)
...
#        grid_layout.addWidget(self.LaplacianSharpnessSlider, 1, 12, 3, 1)
        ...
        #--------------------------------------------------------------------------------------------------------
        # self.SpectrometerWidget = SpectrometerWidget.SpectrometerWidget()
        
        # self.SpectrometerWidget.TakeSpectrumButton.clicked.connect(lambda _ : self.SpecPlot.setData(self.SpectrometerWidget.takeSpectrum(), pen=(0,0,255)))
        ...
        layout.addWidget(QLabel('PL Filters'), 7, 0, 1, 1)
        
        self.MedianEnableBlur = QCheckBox('Blur')
        layout.addWidget(self.MedianEnableBlur, 7, 1, 1, 1)
        
        self.GaussianEnableBlur = QCheckBox('Gaussian')
        layout.addWidget(self.GaussianEnableBlur, 7, 2, 1, 1)
        
        # layout.addWidget(QLabel('Measure for '), 9, 0,1,1)
        
        self.MeasurementLengthLabel = QLabel("Line length: ?")
        layout.addWidget(self.MeasurementLengthLabel, 12, 0, 1, 1)
        ...
        layout.addWidget(self.ObjectiveComboBox, 8, 1, 1, 2)
        
        self.ObjectiveLabel = QLabel('Current Objective:')
        layout.addWidget(self.ObjectiveLabel, 8, 0, 1, 1)
        ...
        layout.addWidget(self.WhiteLightBlueLightSliderComboBox, 9, 1, 1, 2)
        
        self.SliderLabel = QLabel('Current Slider:')
        layout.addWidget(self.SliderLabel, 9, 0, 1, 1)
        ...
        layout.addWidget(self.FilterComboBox, 10, 1, 1, 2)
        
        self.FilterLabel = QLabel('Current Filter:')
        layout.addWidget(self.FilterLabel, 10, 0, 1, 1)
        
        btn = QPushButton('Open image externally')
        btn.clicked.connect(self.openImageExternally)
        layout.addWidget(btn, 13, 0, 1, 4)
        
        self.ShutterLabel = QLabel('Laser Shutter:')
        layout.addWidget(self.ShutterLabel, 11, 0, 1, 1)
        ...
        layout.addWidget(self.ShutterComboBox, 11, 1, 1, 2)
...
#        self.MeasurementLengthLabel.setText("Estimated length: "+str(np.round(L*calibrationFactor,4)))
        ...
        if self.WhiteLightBlueLightSliderComboBox.currentIndex() == 0:
            calibrationNormalized = 130/1611.2541*50.0*100/97.8
        if self.WhiteLightBlueLightSliderComboBox.currentIndex() == 2:
