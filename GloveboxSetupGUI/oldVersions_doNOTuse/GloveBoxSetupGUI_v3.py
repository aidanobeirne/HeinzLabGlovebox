    ...
    def eventLoop(self):
        ...
        if self.WhiteLightBlueLightSliderComboBox.currentIndex() == 2:  
            ...
            if len(imageWL) > 0:
                _ViewBox = self.ImageView.getView()
                _ViewBoxState = _ViewBox.getState()
                self.ImageView.setImage(imageWL, autoLevels=False, levels=(self.ColorScaleMin.value(), self.ColorScaleMax.value()), autoHistogramRange=False)
                _ViewBox.setState(_ViewBoxState)
...
#        self.Camera = Grasshopper3Cam.Grasshopper3Cam()
        ...
        self.WhiteLightCamera = ICMeasureCamera.ICMeasureCam(name='DFK 33UX265 24910240')
...
#        self.MeasurementLengthLabel.setText("Estimated length: "+str(np.round(L*calibrationFactor,4)))
        ...
        self.MeasurementLengthLabel.setText("Estimated length: "+str(np.round(L*calibrationFactor,4)) + " μm")
