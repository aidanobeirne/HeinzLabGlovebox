...
class GloveBoxSetupWindow(QMainWindow):
    ...
    def eventLoop(self):
        ...
        if self.RCBModeOn:
            ...
            # saveSliderPos = self.WhiteLightBlueLightSliderComboBox.currentIndex()
        ...
        # elif self.RCBModeOn==False and self.counter !=0:
        #     self.Camera.startImageAcquisition()
        #     self.WhiteLightCamera.startImageAcquisition()
        #     self.WhiteLightBlueLightSliderComboBox.setCurrentIndex(saveSliderPos)
...
#        grid_layout.addWidget(self.LaplacianSharpnessSlider, 1, 12, 3, 1)
        ...
        PLMap.addWidget(QLabel('Size Y ('+ u"\u03bcm" + '):'), 0, 0, 1, 1)
        ...
        PLMap.addWidget(QLabel('Size X ('+ u"\u03bcm" + '):'), 1, 0, 1, 1)
    ...
    def openImageExternally(self):
        ...
        MeasurementLine2 = pg.LineSegmentROI([[10, 10], [120,10]], pen = (255, 0, 0))
        MeasurementLine2.sigRegionChanged.connect(lambda: self.updateMeasurementLine2(MeasurementLine2, text, whiteLightBlueLightFactor, objectiveFactor))
...
#        self.MeasurementLengthLabel.setText("Estimated length: "+str(np.round(L*calibrationFactor,4)))
    ...
    #Helen's ugly edit
    def updateMeasurementLine2(self, MeasurementLine2, text, whiteLightBlueLightFactor, objectiveFactor):
        ...
        text.setText('Estimated length: ' + str(np.round(L*calibrationFactor,4)) + u" \u03bcm")
    ...
    def startPLMapScan(self):
        if self.SpectrometerWidget.SpectrometerWidget != None:        
            ...
            H = float(self.MapHeight.value())*0.001
            H_px = self.MapHeightPixel.value()
                
            W = float(self.MapWidth.value())*0.001
